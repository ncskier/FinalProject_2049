...
class SolvePuzzleViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func updatePuzzleFirebasePuzzleUsersCorrect() {
        ...
        let puzzleUsersCorrectReference = puzzleReference.child("usersCorrect")
        puzzleUsersCorrectReference.setValue(puzzle.usersCorrect, withCompletionBlock: {(error, firebaseRef) in
            
            if (error != nil) {
                print("Error updating users correct to firebase: \(String(describing: error))")
                
                // Alert User of error
                let errorAlertController = UIAlertController(title: "Error Updating Puzzle Users Correct", message: "\(String(describing: error))", preferredStyle: .alert)
                let dismissAlertAction = UIAlertAction(title: "Dismiss", style: .default, handler: nil)
                errorAlertController.addAction(dismissAlertAction)
    ...
    func locationManager(_ manager: CLLocationManager, didFinishDeferredUpdatesWithError error: Error?) {
        print("Did finish deferring updates with error: \(error!.localizedDescription)")
    }
